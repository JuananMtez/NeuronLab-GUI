...
const TrainingForm = ({csv, experiment}) => {
    ...
    const handleTrainingBtn = () => {
        ...
        let data = {
            name: name,
            csvs: list,
            testing_data: percent.testing_data,
            training_data: percent.training_data,
        ...
        if (learning === 'machine')
            return (
                <Grid item xs={12} sx={{mt: '2vh'}}>
                    <Stack direction="row" spacing={3}>
                        <CustomSelect renderValue={o => renderValue(o, 'Algorithm')}>
                            <StyledOption onClick={() => setAlgorithmMachine('KNN')} value={'KNN'}>KNN</StyledOption>
                            <StyledOption onClick={() => setAlgorithmMachine('Random Forest')} value={'Random Forest'}>Random
                                Forest</StyledOption>
                            <StyledOption onClick={() => setAlgorithmMachine('SVM')} value={'SVM'}>SVM</StyledOption>
                        </CustomSelect>
                        {
                        ...
                        }
                        {
                            ...
                            <CustomSelect renderValue={o => renderValue(o, 'Kernel')} value={svm} onChange={setSvm}>
                                <StyledOption onClick={() => setSvm('linear')} value={'linear'}>linear</StyledOption>
                                <StyledOption onClick={() => setSvm('poly')} value={'poly'}>poly</StyledOption>
                                <StyledOption onClick={() => setSvm('rbf')} value={'rbf'}>rbf</StyledOption>
                            </CustomSelect>
                        }
                    </Stack>
            ...
            return (
                <>
                    <Grid item xs={12} sx={{mt: '2vh'}}>
                        <Stack direction="row">
                            <CustomSelect renderValue={o => renderValue(o, 'Optimizers')}>
                                <StyledOption onClick={() => setModelData({...modelData, optimizer: 'sgd'})} value={'sgd'}>SGD</StyledOption>
                                <StyledOption onClick={() => setModelData({...modelData, optimizer: 'adam'})} value={'adam'}>Adam</StyledOption>
                            </CustomSelect>
                            <Box sx={{ml: '2vh'}}></Box>
                            <CustomSelect renderValue={o => renderValue(o, 'Loss')}>

                                <StyledOption onClick={() => setModelData({...modelData, loss: 'binary_crossentropy'})} value={'binary_crossentropy'}>Binary Crossentropy</StyledOption>

                            </CustomSelect>
                            <Box sx={{ml: '2vh'}}></Box>
                            <CustomSelect renderValue={o => renderValue(o, 'Learning Rate')}>
                                <StyledOption onClick={() => setByDefault('default')} value={'default'}>Default</StyledOption>
                                <StyledOption onClick={() => setByDefault('manual')} value={'manual'}>Manual</StyledOption>
                            </CustomSelect>
                    ...
                    <Grid item xs={12} sx={{mt: '2vh'}}>
                        <CustomSelect renderValue={o => renderValue(o, 'Type')}>
                            <StyledOption onClick={() => setLayer({...layer, type: 'dense'})} value={'dense'}>Dense</StyledOption>
                        </CustomSelect>
                    </Grid>
                    {
                        layer.type !== '' &&
                        <>
                            <Grid item xs={12} sx={{mt: '4vh'}}>
                                <Stack direction="row">
                                    ...
                                    />
                                    <Box sx={{ml: '2vh'}}></Box>
                                    <CustomSelect renderValue={o => renderValue(o, 'Activation Function')}
                                                  >
                                        <StyledOption onClick={() => setLayer({...layer, activation_func: 'softmax'})} value={'softmax'}>Softmax</StyledOption>
                                        <StyledOption onClick={() => setLayer({...layer, activation_func: 'relu'})} value={'relu'}>Relu</StyledOption>
                                        <StyledOption onClick={() => setLayer({...layer, activation_func: 'sigmoid'})} value={'sigmoid'}>Sigmoid</StyledOption>
                                    ...
                                    </CustomSelect>
                                    {
                                        ...
                                        <>
                                            ...
                                            <Box sx={{ml: '2vh'}}></Box>
                                            <CustomSelect renderValue={o => renderValue(o, 'Kernel Initializer')}>
                                                <StyledOption onClick={() => setLayer({...layer, kernel_initializer: 'uniform'})} value={'uniform'}>Uniform</StyledOption>
                                                <StyledOption onClick={() => setLayer({...layer, kernel_initializer: 'random_normal'})} value={'random_normal'}>Random_Normal</StyledOption>
                    ...
                    {layers.map((e, index) => (
                        <Grid key={index} item xs={12} sx={{mt: 3}}>
                            <Chip onDelete={ev => handleDeleteChip(ev, index)} sx={{color: 'white'}}
                                  label={`${index + 1}. ${JSON.stringify(e, null, 4)}`}/>
                        </Grid>
    ...
    return (
        <Grid container>
            <Grid item xs={12}>
                <CustomSelect renderValue={o => renderValue(o, 'Learning Method')}>
                    <StyledOption onClick={() => setLearning('machine')} value={'machine'}>Machine Learning</StyledOption>
                    <StyledOption onClick={() => setLearning('deep')} value={'deep'}>Deep Learning</StyledOption>
                </CustomSelect>
            </Grid>
            {getFormLearning()}
